...
function Login(){
    ...
    return(
        <div className="relative">
            ...
            <div className={`notefication2 border-b  absolute ${close? "-top-40":"-top-3"}  ${msg.error!=null? "-top-3": "-top-20"} ${msg.error == false ? "border-green-400" :"border-red-300"} transition-all rounded-lg text-neutral-800 -translate-1/2 left-1/2 z-40 w-[350px] bg-white shadow-lg shadow-neutral-300/20`}>
                ...
                <form onSubmit={send} method="post" className="contents">
                    <div className="inps flex flex-col py-4 w-full gap-4">
                        <div className="inp-1 flex relative items-center flex-row">
                            <i className={`fa fa-at ${faucus.facus1 ? "text-violet-400":"text-neutral-600"}`}></i>
                            <span className={`absolute -translate-1/2 ${faucus.facus1 ? "top-0 left-15 text-neutral-600 text-sm":"top-1/2 left-15"} transition-all  text-neutral-400 z-[1] `}> Name @ </span>
                            <input onChange={(e)=>Set_data_user((provid)=>({...provid,user:e.target.value}))} onFocus={()=>SetFaucus((prev)=>({...prev,facus1:true}))} onBlur={(e)=> SetFaucus((prev)=> ({...prev,facus1: e.target.value?true:false}))} className={`w-full p-1 px-3 border-b ${faucus.facus1 ? "border-violet-400":"border-neutral-400"} outline-none relative z-[3]`} type="text" />
                        </div>
                        <div className="inp-1 flex relative items-center flex-row">
                            ...
                            <input onChange={(e)=>Set_data_user((provid)=>({...provid,pass:e.target.value}))} onFocus={()=>SetFaucus( (prev)=>({...prev, facus2:true}))} onBlur={(e)=> SetFaucus((prev)=> ({...prev,facus2:e.target.value ? true:false}))} className={`w-full p-1 px-3 border-b ${faucus.facus2 ? "border-violet-400":"border-neutral-400"} outline-none relative z-[3]`} type={`${show?"text":"password"}`} />
